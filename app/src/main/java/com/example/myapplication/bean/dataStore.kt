...
class dataStore(val context: Application) {


    companion object {
        @Volatile
        private var INSTANCE:dataStore? = null
        fun getDataStore(context:Application):dataStore{
            return INSTANCE?: synchronized(this){
                val instance = dataStore(context)
                INSTANCE = instance
                instance
            }
        }
    ...
    fun screeenRotation():Int{
        return getData("屏幕旋转",1)
    }
    fun UAGet():Int{
        return getData("浏览器标识(UA)",0)
    }
    fun UASet(data:Int){
        return putData("浏览器标识(UA)", data)
    }

    fun urlGet():String{
        return getData(key  ="url", defaultValue = "主页")
    }

    fun urlPut(value:String){
        return putData(key = "url", value = value)
    }
    /**
     * 存放Int数据
     */
